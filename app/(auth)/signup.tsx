    ...
    <SafeAreaView>
      <View className="min-h-screen animate-fade-in flex-row">
        ...
        <View className="relative hidden w-1/2 items-center justify-center overflow-hidden bg-[#7F56D9] transition-colors duration-700 lg:flex lg:flex-row">
          ...
          <View className="absolute inset-0 bg-gradient-to-br from-[#7F56D9] to-[#6941C6]"></View>
          <View className="absolute -right-24 -top-24 h-96 w-96 rounded-full bg-white/5 blur-3xl"></View>
          <View className="absolute bottom-0 left-0 h-1/2 w-full bg-gradient-to-t from-black/10 to-transparent"></View>
          ...
          <View className="relative z-10 w-full max-w-lg px-12 text-center">
            ...
            {/* Text Content */}
            <View key={`text-${activeSlide}`} className="min-h-[140px] animate-fade-in">
              <Text className="mb-4 text-3xl font-bold text-white">{currentSlide.title}</Text>
              ...
              {slides.map((_, idx) => (
                <Pressable
                  ...
                  className={`h-2 w-2 rounded-full transition-all duration-300 ${
                    activeSlide === idx ? 'w-6 bg-white' : 'bg-white/30 hover:bg-white/50'
                  }`}
                />
              ))}
            </View>
